...
import { DataServiceOptions } from 'src/app/models/data-service-options.model';
...
export class MockDataService extends DataService {
    ...
    request(options: DataServiceOptions): Observable<Response | HttpResponse<object>> {
        switch (options.method) {
            case 'GET': {
                return of(new HttpResponse({ body: mockMap.get(options.url), headers: new HttpHeaders(null), url: options.url, status: 200, statusText: 'ok' }));
                break;
            }
            case 'POST': {
                return of(new HttpResponse({ body: { message: 'success' } , headers: new HttpHeaders(null), url: options.url, status: 200, statusText: 'ok', }));
                break;
            }
            default: {
                return of(new HttpResponse({ body: null, headers: new HttpHeaders(null), url: options.url, status: 400, statusText: 'No Mock can be found for that request type.' }));
                break;
            }
